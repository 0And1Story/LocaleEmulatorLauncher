...
1. 下载预编译版本或自行构建。
<a href="https://github.com/0And1Story/LocaleEmulatorLauncher/releases/latest">
    <img alt="Latest Release" src="https://img.shields.io/github/v/release/0And1Story/LocaleEmulatorLauncher?style=for-the-badge&logo=github" />
</a>
2. 将 `LocaleEmulator.exe` 放在任意目录，推荐直接放到LocaleEmulator的安装目录（目录下可以直接看到LEProc.exe）。
...
4. 如果想要创建快捷方式，可以将目标程序直接拖到 `CreateLEShortcut.exe` 上，会在桌面创建一个快捷方式，双击即可启动。
5. 命令行等高级用法请参考下文。
