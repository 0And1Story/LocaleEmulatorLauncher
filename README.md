...
<p>
  ...
  <img alt="C++23" src="https://img.shields.io/badge/C%2B%2B-23-00599C?logo=c%2B%2B&logoColor=white" />
  <img alt="CMake" src="https://img.shields.io/badge/CMake-3.20%2B-064F8C?logo=cmake&logoColor=white" />
  <a href="https://github.com/0And1Story/LocaleEmulatorLauncher/releases/latest">
    <img alt="Latest Release" src="https://img.shields.io/github/v/release/0And1Story/LocaleEmulatorLauncher?logo=github&color=brightgreen" />
  </a>
  <a href="https://github.com/0And1Story/LocaleEmulatorLauncher/blob/main/LICENSE">
    <img alt="License" src="https://img.shields.io/github/license/0And1Story/LocaleEmulatorLauncher" />
  </a>
</p>
...
> [!IMPORTANT]
> 请先确保已安装 [Locale Emulator](https://github.com/xupefei/Locale-Emulator/releases/latest)，并记住其安装路径（或将其添加到系统 PATH 中）。

1. 下载预编译版本或自行构建。<p><a href="https://github.com/0And1Story/LocaleEmulatorLauncher/releases/latest"><img alt="Latest Release" src="https://img.shields.io/github/v/release/0And1Story/LocaleEmulatorLauncher?style=for-the-badge&logo=github&color=brightgreen" /></a></p>
3. 将 `LocaleEmulator.exe` 放在任意目录，推荐直接放到LocaleEmulator的安装目录（目录下可以直接看到LEProc.exe）。
4. 将想要启动的目标程序路径（`.exe` 或 `.lnk`）拖到 `LocaleEmulator.exe` 上即可直接启动。
    - 可能会弹出一个窗口选择 Profile（如果需要），选择后配置会记录下来，后续不需要再选择。
    - 如果不想每次都选择，可以通过在启动器目录使用 `--config` 参数进入交互配置向导，提前写入默认配置。
5. 如果想要创建快捷方式，可以将目标程序直接拖到 `CreateLEShortcut.exe` 上，会在桌面创建一个快捷方式，双击即可启动。
6. 命令行等高级用法请参考下文。
...
- 提供 `CreateLEShortcut.exe`，可一键在桌面创建调用启动器的快捷方式
...
```powershell
CreateLEShortcut.exe <target.exe|target.lnk>
```

> [!NOTE]
> 说明：
> - 只接受 1 个参数（目标 `.exe` 或 `.lnk`），不支持其他配置参数。
> - 运行后在桌面创建快捷方式，快捷方式目标为 `LocaleEmulator.exe`，参数为输入的目标路径。
> - 快捷方式名称使用目标程序名。
> - 快捷方式图标使用目标程序图标（若传入 `.lnk`，会先解析到其真实目标）。
...
默认值：
- `InstallPath`：自动发现（启动器目录 -> PATH -> 常见安装目录）
- `Mode`：默认 `runas`
- `ProfileGuid`：无默认，缺失时 `runas` 模式会触发 Profile 选择
...
- `build\Release\CreateLEShortcut.exe`
